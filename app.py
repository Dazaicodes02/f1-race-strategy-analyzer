...
# ── Pre-fetched cache bootstrap ─────────────────────────────────────────
# F1's live-timing servers block requests from most cloud hosting platforms
# (Streamlit Cloud, and likely others) with a 403 — this is out of the app's
# control. The fix: fetch race data once on a machine that CAN reach F1's
# servers (a personal computer), zip up the resulting f1_cache folder, and
# upload it as a GitHub Release asset (GitHub supports up to 2GB per file
# there, far more than git's 100MB-per-file limit, and with no dependency on
# Git LFS — which isn't guaranteed to be supported by every hosting
# platform's build process).
#
# On startup, if no local cache exists yet, the app downloads and unzips
# that release asset automatically — a plain HTTPS request, which is not
# blocked the way direct requests to F1's own servers are.
#
# TO USE THIS: after building your local f1_cache folder with the races you
...
def fig_degradation(clean_laps: pd.DataFrame, year, grand_prix, team_colors, x_mode="tyre_age"):
    ...
    if x_mode == "lap_number" and not clean_laps.empty:
        ...
        # Detect laps where NONE of the selected drivers have data — this is
        # a race-wide gap (Safety Car / VSC / Red Flag period got filtered
        # out along with the individual pit in/out laps), not any single
        # driver's pit stop. Left unexplained, this just looks like a broken
        # chart, so it's shaded and labeled instead of left blank.
        present_laps = set(clean_laps["LapNumber"].unique())
        min_lap, max_lap = int(clean_laps["LapNumber"].min()), int(clean_laps["LapNumber"].max())
        missing_laps = [lap for lap in range(min_lap, max_lap + 1) if lap not in present_laps]
        shared_gap_ranges = []
        if missing_laps:
            start = prev = missing_laps[0]
            for lap in missing_laps[1:]:
                if lap == prev + 1:
                    prev = lap
                else:
                    shared_gap_ranges.append((start, prev))
                    start = prev = lap
            shared_gap_ranges.append((start, prev))
        # Only shade gaps wide enough to matter — a 1-2 lap coincidental gap
        # isn't worth a label, but 3+ laps missing for every driver at once
        # is a real, notable interruption to the race.
        shared_gap_ranges = [(s, e) for s, e in shared_gap_ranges if e - s + 1 >= 3]

        for gstart, gend in shared_gap_ranges:
            fig.add_vrect(
                x0=gstart - 0.5, x1=gend + 0.5,
                fillcolor="rgba(255,255,255,0.07)", line_width=0, layer="below",
                annotation_text="No green-flag laps here for anyone —<br>Safety Car / VSC / Red Flag likely",
                annotation_position="top", annotation_font_size=10,
                annotation_font_color="#bbb",
            )

        def _in_shared_gap(lap_before, lap_after):
            # True if the pit-stop's [lap_before, lap_after] interval overlaps
            # any shared gap range at all (standard interval-overlap check).
            return any(not (lap_after < gstart or lap_before > gend) for gstart, gend in shared_gap_ranges)
        ...
        for driver, driver_data in clean_laps.groupby("Driver"):
            ...
            for s_prev, s_next in zip(stints_sorted[:-1], stints_sorted[1:]):
                ...
                # Skip drawing an individual pit marker if this gap is really
                # part of a shared, already-labeled race-wide stoppage —
                # showing both would just clutter the same spot redundantly.
                if _in_shared_gap(lap_before, lap_after):
                    continue
